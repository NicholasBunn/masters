...
def sendMetrics(func):
    ...
    def wrapper(*args, **kw):
        ...
        if isinstance(args[3], grpc._server._Context):
            ...
            serviceName, methodName = str(serviceMethod).rsplit('/')[1::] # ToDo: add servicename to specify which call is being made within the service
        else:
        ...
        try:
            ...
            pushToPrometheus(args[0].c, args[0].g, args[0].h, responseTime, args[0].address, methodName, args[0].registry)
        return result
    return wrapper
